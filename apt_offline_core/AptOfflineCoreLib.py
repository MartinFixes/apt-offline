...
class FetchBugReports:
    def __init__(
        self, apt_bug_file_format, ArchiveFile=None, lock=False, DownloadDir=None
    ):
        self.bugsList = []
        self.lock = lock
        ...
        if num_of_bugs:
            ...
            for eachBug in self.bugs_list:
                ...
                try:
                    ...
                    log.warn(
                        "Failed to download bug report for %s\nWill continue to download others\n"
                        % (eachBug)
                    )
                    log.err(traceback.format_exc())
                    continue
                ...
                self.fileName = os.path.join(
                    tempfile.gettempdir(),
                    PackageName + "{}" + str(eachBug) + "{}" + self.apt_bug,
                )
                ...
                if self.ArchiveFile:
                    if self.AddToArchive(self.ArchiveFile, self.fileName):
                        ...
                        log.warn(
                            "%s failed to be added to file %s\n"
                            % (self.fileName, self.ArchiveFile)
                        )
...
class AptManip(ExecCmd):
    def __init__(
        self, OutputFile, Simulate=False, AptType="apt-get", AptReinstall=False
    ):
    ...
    def InstallSrcPackages(self, SrcPackageList, ReleaseType, BuildDependency):
        ...
        if self.apt == "apt-get":
            return self.__AptGetInstallSrcPackages(
                SrcPackageList, ReleaseType, BuildDependency
            )
        elif self.apt == "apt":
            return self.__AptInstallSrcPackages(
                SrcPackageList, ReleaseType, BuildDependency
            )
        elif self.apt == "python-apt":
            return self.__AptInstallSrcPackages(
        ...
        if self.ShouldSimulate is True:
            ...
            for localFile in os.listdir(apt_update_target_path):
                if localFile.endswith(".gpg.reverify"):
                    ...
                    os.rename(localFile, os.path.join(
                        apt_update_final_path + sig_file))

    def __AptUpdate(self):
        log.msg("Gathering details needed for 'update' operation\n")
        if (
            self.ExecSystemCmd(
                ["/usr/bin/apt", "-qq", "--print-uris", "update"], self.WriteTo
            )
            is False
        ):
    ...
    def __AptGetUpdate(self):
        ...
        if (
            self.ExecSystemCmd(
                ["/usr/bin/apt-get", "-q", "--print-uris", "update"], self.WriteTo
            )
            is False
        ):
            log.verbose("FATAL: Something is wrong with the apt system.\n")
    ...
    def __PythonAptUpdate(self):
        ...
        for item in acquire.items:
            ...
            log.verbose(
                "Writing string %s %s %d %s to file %s\n"
    ...
    def __PythonAptUpgrade(self, UpgradeType="upgrade", ReleaseType=None):
        ...
        for pkg in cache.get_changes():
            log.verbose(
                "Generable data is: %s %s %d %s\n"
                % (
                    pkg.candidate.uri,
                    pkg.candidate.filename.split("/")[-1],
                    pkg.candidate.size,
                    pkg.candidate.md5,
                )
            )
    ...
    def __AptInstallSrcPackages(
        self, SrcPackageList=None, ReleaseType=None, BuildDependency=False
    ):
        self.ReleaseType = ReleaseType

        log.msg(
            "Gathering installation details for source package %s\n" % (
                SrcPackageList)
        )
        ...
        if self.ReleaseType is not None:
            ...
            cmd.append(self.ReleaseType)
            cmdBuildDep = ["/usr/bin/apt", "-qqq",
                           "--print-uris", "build-dep", "-t"]
        ...
        if BuildDependency:
            log.msg(
                "Generating Build-Dependency for source packages %s.\n"
                % (SrcPackageList)
            )
    ...
    def __AptGetInstallSrcPackages(
        self, SrcPackageList=None, ReleaseType=None, BuildDependency=False
    ):
        self.ReleaseType = ReleaseType
        ...
        if self.ReleaseType is not None:
            ...
            cmd.append(self.ReleaseType)
            cmdBuildDep = ["/usr/bin/apt-get", "-qq",
                           "--print-uris", "build-dep", "-t"]
            cmdBuildDep.append(self.ReleaseType)
        else:
            cmd = ["/usr/bin/apt-get", "-qq", "--print-uris", "source"]
            cmdBuildDep = ["/usr/bin/apt-get",
                           "-qq", "--print-uris", "build-dep"]
...
class APTVerifySigs(ExecCmd):
    def __init__(self, gpgv=None, keyring=None, Simulate=False):
        ...
        if PythonApt is True:
            apt_pkg.init()
            self.defaultPaths.append(
                apt_pkg.config.find_dir("Dir::Etc::trustedparts"))
            self.defaultPaths.append(
            ...
            log.verbose(
                "trusted is %s and trustedparts is %s\n" % (
...
class LockAPT:
    ...
    def __init__(self, lists, packages):
        try:
            self.listLock = os.open(
                lists, os.O_RDWR | os.O_TRUNC | os.O_CREAT, 0o640)
            self.pkgLock = os.open(packages, os.O_RDWR |
                                   os.O_TRUNC | os.O_CREAT, 0o640)
        except Exception:
            log.verbose(traceback.format_exc())
...
class DownloadFromWebDisplay(AptOfflineLib.ProgressBar, GenericDownloadFunction):
    ...
    def __init__(self, width, total_items):
        ...
        AptOfflineLib.ProgressBar.__init__(
            self, width=width, total_items=total_items)
...
def fetcher(args):
    ...
    if Str_ProxyHost:
        if Str_ProxyPort:
            ...
            proxy_support = urllib.request.ProxyHandler(
                {
                    "http": Str_ProxyHost + ":" + str(Str_ProxyPort),
                    "https": Str_ProxyHost + ":" + str(Str_ProxyPort),
                }
            )
            opener = urllib.request.build_opener(proxy_support)
            urllib.request.install_opener(opener)
            log.verbose(
                "Proxy successfully set up with Host %s and port %s\n"
                % (Str_ProxyHost, str(Str_ProxyPort))
            )
        else:
            proxy_support = urllib.request.ProxyHandler(
                {"http": Str_ProxyHost, "https": Str_ProxyHost}
            )
            opener = urllib.request.build_opener(proxy_support)
            urllib.request.install_opener(opener)
            log.verbose(
                "Proxy successfully set up with Host %s and default port\n"
                % (Str_ProxyHost)
            )
    ...
    if (Str_HttpsCertFile and Str_HttpsKeyFile) or Bool_DisableCertCheck:
        ...
        if Str_HttpsCertFile and Str_HttpsKeyFile:
            log.verbose(
                "cert-file:" + Str_HttpsCertFile + " key-file:" + Str_HttpsKeyFile
            )
        ...
        urllib.request.install_opener(opener)
        log.verbose(
            "SSL Client Authentication successfully set up with certificate file %s and key file %s\n"
    ...
    if Str_HttpBasicAuth:
        ...
        try:
            for authstr in Str_HttpBasicAuth:
                ...
                log.verbose(
                    "Added user %s with pass %s auth to domain %s\n"
    ...
    if Str_CacheDir:
        if os.path.isdir(Str_CacheDir) is False:
            log.err(
                "WARNING: cache dir %s is incorrect. Did you give the full path ?\n"
                % (Str_CacheDir)
            )
            sys.exit(1)
    ...
    if Str_DownloadDir:
        ...
        if os.access(tempdir, os.W_OK):
            pidname = os.getpid()
            randomjunk = (
                "".join(chr(random.randint(97, 122)) for x in range(5))
                if guiBool
                else ""
            )
            ...
            tempdir = os.path.join(
                tempdir, "apt-offline-downloads-" + str(pidname) + randomjunk
            )
            os.mkdir(tempdir)
    ...
    class FetcherClass(
        DownloadFromWeb,
        AptOfflineLib.Archiver,
        AptOfflineLib.Checksum,
        AptOfflineLib.FileMgmt,
        FetchBugReports,
    ):
        def __init__(self, *args, **kwargs):
            DownloadFromWeb.__init__(
                self, width=kwargs.pop("width"), total_items=kwargs.pop("total_items")
            )
            ...
            FetchBugReports.__init__(
                self, apt_bug_file_format, Str_BundleFile, lock=kwargs.get(
                    "lock")
            )
                ...
                for pkgFile in pkgHandle.filelist:
                    if constChangelog in pkgFile:
                        chlogFile = tempfile.NamedTemporaryFile(
                            "r+", buffering=-1, encoding="utf-8", dir=None, delete=True
                        )
                        pkgLogFile = open(
                            os.path.join(
                                tempfile.gettempdir(), pkgHandle.pkgname + ".changelog"
                            ),
                            "w",
                            encoding="utf-8",
                        )
                        ...
                        try:
                            chlogFile.writelines(
                                pkgHandle.data_content(pkgFile))
                        except TypeError:
                            log.warn(
                                "Couldn't extract changelog for package %s\n"
                                % (pkgHandle.pkgname)
                            )
                            log.verbose(traceback.format_exc())
                            break
                        ...
                        installedVersion_changelog_line = "%s (%s) " % (
                            srcname,
                            installedVersion,
                        )
    ...
    # INFO: This key/val dict contains record of installed packages
    PackageInstalledVersion = {}
    ...
    if Str_GetArg is not None:
        ...
        for item in raw_data_list:
            if item.startswith("Changelog/"):
                ...
                log.verbose(
                    "Added package %s with version %s to dict\n" % (
                        pkgName, pkgVersion)
                )
            else:
                ...
                try:
                    ...
                    if not ItemURL.startswith(("http", "https", "ftp", "file")):
                        ...
                        FetchData["Item"].append(
                            ReleaseItemURL + " " +
                            ReleaseItemFile + " " + str(ItemSize)
                        )
                        log.verbose("Printing Release URL/Files\n")
                        log.verbose("%s %s" %
                                    (ReleaseItemURL, ReleaseItemFile))
                    FetchData["Item"].append(item)
                except IndexError:
                    log.err(
                        "Failed parisng file: %s, while processing line: %s\n"
                        % (Str_GetArg, item)
                    )
                    log.err(traceback.format_exc())
                    sys.exit(1)
    ...
    FetcherInstance = FetcherClass(
        Bool_DisableMD5Check,
        Str_BundleFile,
        Bool_BugReports,
        Str_DownloadDir,
        Str_CacheDir,
    ...
    def DataFetcher(request, response, func=FetcherInstance.find_first_match):
        ...
        if url.endswith(".deb"):
            try:
                ...
                log.err(
                    "Not getting a package name here is problematic. Better bail out.\n"
                )
                sys.exit(1)
            ...
            try:
                ...
                PackageVersion = "NA"
                log.verbose(
                    "Weird!! Package version not present. Is it really a deb file?\n"
                )
                ...
                if FetcherInstance.verifyPayloadIntegrity(full_file_path, checksum):
                    ...
                    if PackageName in list(PackageInstalledVersion.keys()):
                        FetcherInstance.buildChangelog(
                            full_file_path, PackageInstalledVersion[PackageName]
                        )
                else:
                    log.verbose(
            ...
            if pkgFile.endswith(".gpg") or pkgFile.endswith("Release"):
                ...
                pkgFile.endswith(".dsc")
                or "orig.tar" in pkgFile
                or "debian.tar" in pkgFile
            ):
                # Same as above.
    ...
    if not guiTerminateSignal:
        ConnectThread = AptOfflineLib.MyThread(
            DataFetcher, requestQueue, responseQueue, Int_NumOfThreads
        )
...
def installer(args):
    class InstallerClass(
        AptOfflineLib.Archiver, AptOfflineLib.Checksum, AptOfflineLib.FileMgmt, LockAPT
    ):
        def __init__(self, args):
            ...
            if not os.access(self.tempdir, os.W_OK):
                log.err(
                    "Temporary path %s in not writable. Some functionality may fail\n"
                )
                return False
            ...
            if MagicLib is False:
                ...
                return False
            self.magicMIME = AptOfflineMagicLib.open(
                AptOfflineMagicLib.MAGIC_MIME_TYPE)
            ...
            if self.Str_InstallSrcPath is None:
                ...
                srcDownloadsPath = os.path.join(
                    self.tempdir,
                    "apt-offline-src-downloads-" + str(pidname) + randomjunk,
                )
            ...
            if self.Bool_TestWindows:
                pidname = os.getpid()
                tempdir = os.path.join(
                    self.tempdir, "apt-package-target-path-" + str(pidname)
                )
                ...
                tempdir = os.path.join(
                    self.tempdir, "apt-update-target-path-" + str(pidname)
                )
                ...
                tempdir = os.path.join(
                    self.tempdir, "apt-update-final-path-" + str(pidname)
                )
                ...
                try:
                    if os.geteuid() != 0:
                        log.err(
                            "EACCES: You need superuser privileges to execute this option\n"
                        )
                        sys.exit(13)
                except AttributeError:
                    log.err(
                        "EOPNOTSUPP: Unsupported platform: %s\n" % (
                            platform.platform())
                    )
                    sys.exit(95)
        ...
        def magic_check_and_uncompress(self, archive_file=None, filename=None):
            ...
            retval = False
            if (
                self.magicMIME.file(archive_file) == "application/x-bzip2"
                or self.magicMIME.file(archive_file) == "application/gzip"
                or self.magicMIME.file(archive_file) == "application/x-xz"
            ):
                temp_filename = os.path.join(
                    self.apt_update_target_path, filename + app_name
                )
                if self.magicMIME.file(archive_file) == "application/x-bzip2":
                    retval = self.decompress_the_file(
                        archive_file, temp_filename, "bzip2"
                    )
                    filename = filename.rstrip(".bz2")
                    filename = filename.rstrip(".bzip")
                elif self.magicMIME.file(archive_file) == "application/gzip":
                    retval = self.decompress_the_file(
                        archive_file, temp_filename, "gzip"
                    )
                    filename = filename.rstrip(".tar.gz")
                    filename = filename.rstrip(".gz")
                elif self.magicMIME.file(archive_file) == "application/x-xz":
                    retval = self.decompress_the_file(
                        archive_file, temp_filename, "xz")
                ...
                if retval is True:
                    ...
                    try:
                        ...
                        log.warn(
                            "Failed to unlink temporary file %s. Check respective decompressor library support\n"
                            % (temp_filename)
                        )

            elif (
                self.magicMIME.file(
                    archive_file) == "application/x-gnupg-keyring"
                or self.magicMIME.file(archive_file) == "application/pgp-signature"
                or self.magicMIME.file(archive_file) == "text/PGP"
            ):
                ...
                self.magicMIME.file(archive_file)
                == "application/vnd.debian.binary-package"
                or self.magicMIME.file(archive_file) == "application/x-debian-package"
            ):
                ...
                log.err(
                    "I couldn't understand file %s of type %s.\n"
                ...
                if response == "?":
                    ...
                    if dataType == "file":
                        for filename in zipBugFile.namelist():
                            ...
                            if (
                                found is True
                            ):  # found is True. That means this is a src package
                                shutil.copy2(
                                    archive_file,
                                    os.path.join(
                                        self.Str_InstallSrcPath, filename),
                                )
                                log.msg(
                                    "Installing src package file %s to %s.\n"
                            ...
                            if self.Bool_TestWindows:
                                log.verbose(
                                    "In simulate mode. No locking required\n")
                            ...
                            self.magic_check_and_uncompress(
                    ...
                    if dataType == "dir":
                        if DirInstallPackages(self.Str_InstallArg) is True:
                            ...
                            log.err(
                                "Failed during install operation on %s.\n"
                                % (self.Str_InstallArg)
                            )
                            sys.exit(1)
                    ...
                    if found:
                        if dataType == "file":
                            pydoc.pager(
                                zipBugFile.read(
                                    bug_file_to_display).decode("utf-8")
                            )
                            # Redisplay the menu
                ...
                if localFile.endswith(".gpg"):
                    ...
                    if self.AptSecure.VerifySig(gpgFile, gpgFile.rstrip(".gpg")):
                        ...
                        for line in releaseFile.readlines():
                            ...
                            if (
                                not line.startswith(" ")
                                and self.checksumHeader not in line
                            ):
                                startTrack = False
                            if startTrack:
                                try:
                                ...
                                # Remove the trailing _ underscore
                                aptPkgFile = aptPkgFile[:-1]
                                aptPkgFile = aptPkgFile.split("/")[-1]
                                aptPkgFile = aptPkgFile + "_" + \
                                    files.replace("/", "_")

                                self.checksumList.append(
                                    [checksum, size, aptPkgFile, "sha256"]
                                )
                    else:
                        # Bad sig.
                        log.err(
                            "%s bad signature. Not syncing because in strict mode.\n"
                            % (localFile)
                        )
                        ...
                        for line in releaseFile.readlines():
                            ...
                            if startTrack:
                                ...
                                aptPkgFile = aptPkgFile + "_" + \
                                    files.replace("/", "_")
                                self.checksumList.append(
                                    [checksum, size, aptPkgFile, checksumType]
                                )
                    else:
                        # Bad sig.
                        log.err(
            ...
            for localFile in self.lFileList:
                for checksumItem in self.checksumList:
                    if os.path.basename(localFile) == checksumItem[2]:
                        if InstallerInstance.verifyPayloadIntegrity(
                            localFile, checksumItem[0], checksumItem[3]
                        ):
                            log.verbose(
                                "localFile %s Integrity with checksum %s matches\n"
                                % (localFile, checksumItem[0])
                            )
                            self.lVerifiedWhitelist.append(localFile)
                        else:
                            log.verbose(
                                "localFile %s integrity doesn't match to checksum %s\n"
                                % (localFile, checksumItem[0])
                            )
            ...
            for whitelist_item in verifiedWhiteList:
                for final_item in masterList:
                    if whitelist_item == final_item:
                        self.magic_check_and_uncompress(
                            final_item, os.path.basename(final_item)
                        )
    ...
    if os.path.isfile(installPath):
        ...
        if InstallerInstance.Bool_SkipChangelog:
            ...
            InstallerInstance.displayChangelog(
                InstallerInstance.Str_InstallArg)
            ...
            for filename in zipBugFile.namelist():
                if filename.endswith(apt_bug_file_format):
                    ...
                    for bug_subject_identifier in temp.file.readlines():
                        bug_subject_identifier = bug_subject_identifier.decode(
                            "utf-8")
                        if bug_subject_identifier.startswith("Subject:"):
                            subject = bug_subject_identifier.lstrip(
                                bug_subject_identifier.split(":")[0]
                            )
        ...
        if bugs_number:
            ...
            log.verbose(
                "Great!!! No bugs found for all the packages that were downloaded.\n\n"
            )
            ...
            for filename in zipBugFile.namelist():
                ...
                if (
                    found is True
                ):  # We are src packages. And don't need a lock on the APT Database
                    shutil.copy2(
                        archive_file,
                        os.path.join(
                            InstallerInstance.Str_InstallSrcPath, filename),
                    )
                    log.msg(
                        "Installing src package file %s to %s.\n"
                ...
                if filename.endswith(".deb"):
                    if (
                        InstallerInstance.magicMIME.file(archive_file)
                        == "application/vnd.debian.binary-package"
                        or InstallerInstance.magicMIME.file(archive_file)
                        == "application/x-debian-package"
                    ):
                        debFile = os.path.join(
                            InstallerInstance.apt_package_path, filename
                        )
            ...
            if InstallerInstance.Bool_Untrusted:
                log.err(
                    "Disabling apt gpg check can risk your machine to compromise.\n"
                )
                if not InstallerInstance.installVerifiedList(FileList, FileList):
                    log.err("Failed to synchronize the APT payload\n")
                    sys.exit(1)
            else:
                verifiedList = InstallerInstance.verifyAptFileIntegrity(
                    FileList)
        ...
        def DirInstallPackages(InstallDirPath):
            for eachfile in os.listdir(InstallDirPath):
                ...
                FullFileName = os.path.abspath(
                    os.path.join(InstallDirPath, eachfile))
                ...
                if found is True:
                    shutil.copy2(
                        FullFileName, InstallerInstance.Str_InstallSrcPath)
                    log.msg(
                        "Installing src package file %s to %s.\n"
                        % (filename, InstallerInstance.Str_InstallSrcPath)
                    )
                ...
                if eachfile.endswith(".deb"):
                    InstallerInstance.magic_check_and_uncompress(
        ...
        if InstallerInstance.Bool_Untrusted:
            ...
            for x in os.listdir(installPath):
                if x.endswith(apt_bug_file_format) or x.endswith(".deb"):
                    ...
                    # Do we do a move ??
                    shutil.copy2(x, InstallerInstance.apt_update_final_path)
                    log.verbose(
                        "%s synced to %s\n"
...
def setter(args):
    ...
    if (
        Bool_SetUpdate is False
        and Bool_SetUpgrade is False
        and List_SetInstallPackages is None
    ...
    AptInst = AptManip(Str_SetArg, Simulate=Bool_TestWindows,
                       AptType=args.apt_backend)

    if Bool_SetUpdate:
        if platform.system() in supported_platforms:
            if not Bool_TestWindows and os.geteuid() != 0:
                log.err(
                    "This option requires super-user privileges. Execute as root or use sudo/su\n"
                )
                sys.exit(1)
            else:
            ...
            log.err(
                "This argument is supported only on Unix like systems with apt installed\n"
            )
            sys.exit(1)
    ...
    if Bool_SetUpgrade:
        if platform.system() in supported_platforms:
            ...
            if Str_SetUpgradeType == "upgrade":
                if (
                    AptInst.Upgrade(
                        "upgrade", ReleaseType=Str_SetInstallRelease)
                    is False
                ):
                    sys.exit(1)
                ...
                if (
                    AptInst.Upgrade(
                        "dist-upgrade", ReleaseType=Str_SetInstallRelease)
                    is False
                ):
                    sys.exit(1)
                ...
                if (
                    AptInst.Upgrade(
                        "dselect-upgrade", ReleaseType=Str_SetInstallRelease
                    )
                    is False
                ):
                    sys.exit(1)
    ...
    if List_SetInstallPackages != None and List_SetInstallPackages != []:
        if platform.system() in supported_platforms:
            ...
            if (
                AptInst.InstallPackages(
                    List_SetInstallPackages, Str_SetInstallRelease)
                is False
            ):
                sys.exit(1)
        else:
    ...
    if List_SetInstallSrcPackages != None and List_SetInstallSrcPackages != []:
        if platform.system() in supported_platforms:
            if (
                AptInst.InstallSrcPackages(
                    List_SetInstallSrcPackages, Str_SetInstallRelease, Bool_SrcBuildDep
                )
                is False
            ):
                sys.exit(1)
    ...
    if Bool_Changelog:
        ...
        for eachLine in sigFile.readlines():
            ...
            try:
                ...
                log.verbose(
                    "Cannot find package %s in package cache\n" % (pkgName))
                continue
            except Exception:
                log.err(traceback.format_exc())
                raise
            ...
            log.verbose(
                "Writing to Changelog, pkgName: %s, pkgInstalledVersion %s\n"
    ...
    if not Bool_TestWindows:
        if os.path.getsize(Str_SetArg) == 0:
            log.err("Generated signature file %s is of 0 bytes\n" %
                    (Str_SetArg))
            log.err(
                "This is usually the case when the underneath apt system has no payload to download\n"
            )
            sys.exit(1)
...
def main():
    ...
    global_options.add_argument(
        "--verbose", dest="verbose", help="Enable verbose messages", action="store_true"
    )
    global_options.add_argument(
        "--quiet", dest="quiet", help="Enable quiet mode", action="store_true"
    )

    if float(argparse.__version__) >= 1.1:
        parser = argparse.ArgumentParser(
            prog=app_name,
            description="Offline APT Package Manager" + " - " + version,
            epilog=myCopyright + " - " + terminal_license,
    ...
    parser_set.add_argument(
        "--simulate",
        dest="set_simulate",
        help="Just simulate. Very helpful when debugging",
        action="store_true",
    )
    ...
    parser_set.add_argument(
        "--install-packages",
        dest="set_install_packages",
        help="Packages that need to be installed",
        action="store",
        type=str,
        nargs="*",
        metavar="PKG",
    ...
    parser_set.add_argument(
        ...
        dest="set_install_src_packages",
        help="Source Packages that need to be installed",
        action="store",
        type=str,
        nargs="*",
        metavar="SOURCE PKG",
    ...
    parser_set.add_argument(
        "--src-build-dep",
        dest="src_build_dep",
        help="Install Build Dependency packages for requested source packages",
        action="store_true",
    )

    parser_set.add_argument(
        "--release",
        dest="set_install_release",
        help="Release target to install packages from",
        action="store",
        type=str,
    ...
    parser_set.add_argument(
        "--update",
        dest="set_update",
        help="Generate Signature to update APT Database",
        action="store_true",
    )
    ...
    parser_set.add_argument(
        "--upgrade",
        dest="set_upgrade",
        help="Generate Signature of packages to be upgraded",
        action="store_true",
    )
    ...
    parser_set.add_argument(
        "--generate-changelog",
        dest="generate_changelog",
        help="Generate changelog of the version to be downloaded",
        action="store_true",
    )
    ...
    parser_set.add_argument(
        "--apt-backend",
        dest="apt_backend",
        help="APT backend to use. One of: apt, apt-get, python-apt",
        action="store",
        type=str,
        metavar="apt-get",
        default="apt-get",
    ...
    parser_get.add_argument(
        ...
        "--download-dir",
        dest="download_dir",
        help="Folder path to save files to",
        action="store",
        type=str,
        metavar="apt-downloads",
    )
    ...
    parser_get.add_argument(
        ...
        action="store",
        type=str,
        metavar="/var/cache/apt/archives/",
    )

    parser_get.add_argument(
        "--no-checksum",
        dest="disable_md5check",
    ...
    parser_get.add_argument(
        "--bundle",
        dest="bundle_file",
        help="Bundle output data to a file",
        action="store",
        type=str,
    ...
    parser_get.add_argument(
        ...
        help="Certificate file for https client authentication",
        type=str,
        default=None,
    )
    ...
    parser_get.add_argument(
        "--https-key-file",
        dest="https_key_file",
        help="Certificate key for https client authentication",
        type=str,
        default=None,
    )
    ...
    parser_get.add_argument(
        ...
        dest="http_basicauth",
        action="append",
        help="A user/password encoded URL. Passwords with special characters should be percent encoded",
        type=str,
        default=[],
        metavar="https://username:password@hostname.com/",
    )

    parser_get.add_argument(
        "--disable-cert-check",
        dest="disable_cert_check",
        help="Disable Certificate check on https connections",
        action="store_true",
    )
    ...
    parser_install.add_argument(
        ...
        action="store",
        type=str,
        metavar="apt-offline-download.zip | apt-offline-download/",
    )

    parser_install.add_argument(
        "--simulate",
        dest="install_simulate",
        help="Just simulate. Very helpful when debugging",
        action="store_true",
    )
    ...
    parser_install.add_argument(
        ...
        help="Skip the bug report check",
        action="store_true",
    )
    ...
    parser_install.add_argument(
        "--skip-changelog",
        dest="skip_changelog",
        help="Skip display of changelog",
        action="store_true",
    )
    ...
    parser_install.add_argument(
        ...
        help="Ignore apt gpg signatures mismatch",
        action="store_true",
    )

    parser_install.add_argument(
        "--strict-deb-check",
        dest="strict_deb_check",
